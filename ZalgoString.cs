...
class ZalgoString {
    public string Text;
    public string ZalgoText { get => Generate(Text, Up, Mid, Down, IgnoreZalgo, Rate); set { } }
    public bool Up, Mid, Down, IgnoreZalgo;
    public ushort Rate;

    public ZalgoString(string str, bool up = true, bool mid = false, bool down = true, bool ignoreZalgo = true, ushort rate = 5) {
        Text = str;
        Up = up;
        Mid = mid;
        Down = down;
        IgnoreZalgo = ignoreZalgo;
        Rate = rate;
    }
    ...
    public static implicit operator string(ZalgoString zalgo) {
        return zalgo.ZalgoText;
    }

    public static implicit operator ZalgoString(string text) {
        return new ZalgoString(text, true, false, true, true, 5);
    }
    ...
    private string Generate(string originalStr, bool zalgoUp, bool zalgoMid, bool zalgoDown, bool ignoreZalgoChars, ushort zalgoRate) {
        string ret = "";
        char[] strChars = originalStr.ToCharArray();

        for (int i = 0; i < originalStr.Length; i++) {
            if (!ignoreZalgoChars && IsZalgoChar(strChars[i], zalgoUp ? ZalgoUp.Length : 0, zalgoMid ? ZalgoDown.Length : 0, zalgoDown ? ZalgoMid.Length : 0))
                continue;
